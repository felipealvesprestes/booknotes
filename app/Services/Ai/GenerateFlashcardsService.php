...
class GenerateFlashcardsService
{
    ...
    protected int $maxSourceCharacters;
    ...
    public function __construct()
    {
        ...
        $this->maxSourceCharacters = (int) config('ai.flashcards.pdf.max_characters', 8000);
    }
    ...
    public function generate(
        ...
        $cards = $this->requestFlashcards($messages);

        return $this->normalizeFlashcards($cards, $quantity);
    }
    ...
    public function generateFromContent(
        ...
        $messages = [
            ...
            [
                'role' => 'user',
                'content' => $this->userPromptFromContent(
                    $discipline,
                    $content,
                    $quantity,
                    $user->locale,
     ...
     */
    protected function requestFlashcards(array $messages): array
    {
        if ($this->apiKey === '') {
     ...
     * @return array<int, array{question: string, answer: string, extra?: string|null}>
     */
    protected function normalizeFlashcards(array $cards, int $quantity): array
    {
...
Rules:
- Use only factual content related to the given discipline and provided context.
...
PROMPT;
    ...
    protected function userPromptFromContent(
        Discipline $discipline,
        string $content,
        int $quantity,
        ?string $locale,
        ...
        $language = $this->humanReadableLanguage($locale);
        $preparedContent = $this->prepareSourceContent($content);

        return <<<PROMPT
Language for questions and answers: {$language}
Discipline: {$discipline->title}
Source: Extracted text from the user's PDF. Ground every flashcard ONLY on the content below.
Content:
{$preparedContent}

Generate EXACTLY {$quantity} flashcards using the ideas found in the PDF content above. Avoid inventing facts that are not supported there. Each flashcard must cover one clear idea.
...
PROMPT;
    ...
    protected function prepareSourceContent(string $content): string
    {
        $normalized = Str::of($content)->squish();

        if ($this->maxSourceCharacters > 0) {
            $normalized = $normalized->limit($this->maxSourceCharacters, '...');
        }
